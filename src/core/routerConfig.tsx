...
export function routerConfig(routers: RouterParam[], parentPath = '') {
    ...
    for (let i in routers) {
        ...
        if (auth) {
            list.push(<PrivateRouter key={parentPath + path} auth={auth} path={parentPath + path} exact={exact} render={(props: any) => <Component {...props}>{children}</Component>}></PrivateRouter>)
        } else {
